  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">View the complete history of your repair requests.</p>
      </div>
              ...
              <div key={repair.id} className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between hover:bg-white/[0.02] transition-colors gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-2 line-clamp-1 max-w-md">
                      Issue: {repair.issue}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-4 self-end md:self-center">
                  <div className="text-right hidden sm:block mr-4">
                    ...
                    <div className="text-xs text-muted-foreground">{new Date(repair.created_at).toLocaleDateString()}</div>
                  </div>
                  <a
                    href={`/track?id=${repair.ticket_no}`}
                    className="inline-flex items-center justify-center rounded-md text-sm font-medium border border-border bg-secondary hover:bg-cardccent h-9 px-4 py-2 transition-colors"
                  >
                    Track Status
                  </a>
                </div>
